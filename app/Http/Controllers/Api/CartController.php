...
class CartController extends Controller
{
    ...
    {
        try {
            $cart =  $this->cartService->getCart($this->user,$request->temp_user_id);
    ...
    {
        try {
            if ($this->cartService->emptyCart($this->user,request()->temp_user_id))
    ...
    {
        try {
            $cart = $this->cartService->removeItem($id, $this->user,request()->temp_user_id);
            return new CartResource($cart);
        } catch (Exception $ex) {
            return apiResponse(message: $ex->getMessage());
        }
    ...
    public function store(CartStoreRequest $request,ProductService $productService)
    {
        try {
            ...
            $cart = $this->cartService->addItem(product_id: $request->product_id,quantity: $request->quantity, user: $this->user,temp_user_id: $request->temp_user_id);
            DB::commit();
            return new CartResource($cart);
        } catch (Exception $ex) {
            DB::rollBack();
